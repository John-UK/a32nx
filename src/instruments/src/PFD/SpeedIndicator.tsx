    ...
    render(): VNode {
        return (
            ...
            >
                ...
                <text
                    class="FontLarge MiddleAlign Cyan"
                    x="26.205544"
                    y="82.96"
                >
                    1
...
> {
    ...
    private speedTapeOutlineRef: NodeReference<
        SVGPathElement
    > = FSComponent.createRef();

    private speedTapeElements: NodeReference<
        SVGGElement
    > = FSComponent.createRef();
    ...
    private setOutline() {
        ...
        if (
            this.airSpeed.isFailureWarning() ||
            (this.airSpeed.isNoComputedData() && !this.onGround)
        ) {
            airspeedValue = NaN;
        ...
        const length =
            42.9 +
            Math.max(Math.max(Math.min(airspeedValue, 72.1), 30) - 30, 0);
        this.speedTapeOutlineRef.instance.setAttribute(
            "d",
            `m19.031 38.086v${length}`
        );
    }
        ...
        pf.on("altitudeAr")
            ...
            .handle((a) => {
                ...
                if (
                    ...
                    this.altitude.value < 15000 &&
                    this.flapHandleIndex < 4
                ) {
                    this.vfeNext.instance.classList.remove("HiddenElement");
                } else {
                    this.vfeNext.instance.classList.add("HiddenElement");
        ...
        pf.on("alphaProt")
            ...
            .handle((a) => {
                this.alphaProtRef.forEach((el, index) => {
                    ...
                    const offset =
                        (-elementValue * DistanceSpacing) / ValueSpacing;
        ...
        pf.on("vMax")
            ...
            .handle((vMax) => {
                this.vMaxRef.forEach((el, index) => {
                    const elementValue = vMax + 5.04 * index;
                    const offset =
                        (-elementValue * DistanceSpacing) / ValueSpacing;
                    el.instance.style.transform = `translate3d(0px, ${offset}px, 0px)`;
                });
    ...
    render(): VNode {
        const length =
            42.9 + Math.max(Math.max(Math.min(100, 72.1), 30) - 30, 0);
        return (
            <>
                <g
                    id="FailedGroup"
                    ref={this.failedGroup}
                    class="HiddenElement"
                >
                    <path
                    ...
                    <text
                        id="SpeedFailText"
                        class="Blink9Seconds FontLargest EndAlign Red"
                        x="17.756115"
                        y="83.386398"
                    >
                        SPD
                    ...
                    <path
                        ...
                        ref={this.speedTapeOutlineRef}
                        class="NormalStroke Red"
                        d={`m19.031 38.086v${length}`}
                    />
                </g>
...
class FlapsSpeedPointBugs extends DisplayComponent<{ bus: EventBus }> {
    ...
    onAfterRender(node: VNode): void {
        ...
        sub.on("greenDotSpeed")
            .whenChanged()
            .handle((gd) => {
                this.greenDotBug.instance.style.transform = `translate3d(0px,${getSpeedTapeOffset(
                    gd
                )}px, 0px`;
            });
        sub.on("slatSpeed")
            .whenChanged()
            .handle((sls) => {
                this.slatBug.instance.style.transform = `translate3d(0px,${getSpeedTapeOffset(
                    sls
                )}px, 0px`;
            });
        sub.on("fSpeed")
            .whenChanged()
            .handle((fs) => {
                this.flapsBug.instance.style.transform = `translate3d(0px,${getSpeedTapeOffset(
                    fs
                )}px, 0px`;
            });
    }
...
const getSpeedTapeOffset = (speed: number): number =>
    (-speed * DistanceSpacing) / ValueSpacing;
...
export class AirspeedIndicatorOfftape extends DisplayComponent<{
    ...
    onAfterRender(node: VNode): void {
        ...
        sub.on("speedAr").handle((speed) => {
            let airspeedValue: number;
            if (
                speed.isFailureWarning() ||
                (speed.isNoComputedData() && !this.onGround)
            ) {
                airspeedValue = NaN;
            ...
            if (Number.isNaN(airspeedValue)) {
                ...
                this.offTapeFailedRef.instance.classList.remove(
                    "HiddenElement"
                );
            } else {
                ...
                if (showLower) {
                    this.lowerRef.instance.setAttribute(
                        "visibility",
                        "visible"
                    );
                } else {
...
class SpeedTrendArrow extends DisplayComponent<{
    ...
    onAfterRender(node: VNode): void {
        ...
        sub.on("realTime").handle((_t) => {
            ...
            const airspeedAcc =
                ((clamped - this.previousAirspeed) / deltaTime) * 1000;
            this.previousAirspeed = clamped;

            let filteredAirspeedAcc = this.lagFilter.step(
                airspeedAcc,
                deltaTime / 1000
            );
            filteredAirspeedAcc = this.airspeedAccRateLimiter.step(
                filteredAirspeedAcc,
                deltaTime / 1000
            );
...
class VLsBar extends DisplayComponent<{ bus: EventBus }> {
    ...
    private setVlsPath(vls: number) {
        ...
        const VLsPos =
            ((airSpeed - vls) * DistanceSpacing) / ValueSpacing + 80.818;
        const offset =
            ((vls - this.vlsState.alphaProtSpeed) * DistanceSpacing) /
            ValueSpacing;
    ...
    onAfterRender(node: VNode): void {
        ...
        const sub = this.props.bus.getSubscriber<
            Arinc429Values & PFDSimvars & ClockEvents
        >();
...
class VAlphaLimBar extends DisplayComponent<{ bus: EventBus }> {
    ...
    private setAlphaLimBarPath() {
        if (this.vAlphaLim - this.airSpeed.value < -DisplayRange) {
            ...
            this.VAlimIndicator.instance.setAttribute(
                "d",
                `m19.031 123.56h3.425v${offset}h-3.425z`
            );
        }
    }
...
class SpeedTarget extends DisplayComponent<{ bus: EventBus }> {
    ...
    private handleManagedSpeed() {
        if (this.speedState.isSpeedManaged) {
            ...
            const text = Math.round(this.speedState.managedTargetSpeed)
                .toString()
                .padStart(3, "0");
    ...
    onAfterRender(node: VNode): void {
        ...
        const sub = this.props.bus.getSubscriber<
            PFDSimvars & SimplaneValues & ClockEvents & Arinc429Values
        >();

        sub.on("isSelectedSpeed")
            .whenChanged()
            .handle((s) => {
    ...
    private onFrameUpdate(_realTime: number): void {
        if (this.needsUpdate === true) {
            ...
            if (inRange) {
                ...
                const currentValueAtPrecision =
                    Math.round(this.speedState.speed.value * multiplier) /
                    multiplier;
                const offset =
                    ((currentValueAtPrecision -
                        (this.speedState.isSpeedManaged
    ...
    private handleLowerUpperBound(): boolean {
        ...
        if (this.speedState.speed.value - currentTargetSpeed > DisplayRange) {
            ...
            this.speedState.speed.value - currentTargetSpeed < -DisplayRange &&
            !this.decelActive
        ) {
