...
export const getDisplayIndex = () => {
    const url = document
        .getElementsByTagName("a32nx-pfd")[0]
        .getAttribute("url");
...
export class PFDComponent extends DisplayComponent<PFDProps> {
    ...
    public onAfterRender(node: VNode): void {
        ...
        this.failuresConsumer.register(
            getDisplayIndex() === 1
                ? A320Failure.LeftPfdDisplay
                : A320Failure.RightPfdDisplay
        );

        const sub = this.props.bus.getSubscriber<
            Arinc429Values & ClockEvents
        >();
            ...
            .handle((_t) => {
                ...
                if (
                    ...
                    ((this.pitch.isNormalOperation() &&
                        (-this.pitch.value > 25 || -this.pitch.value < -13)) ||
                        (this.roll.isNormalOperation() &&
                            Math.abs(this.roll.value) > 45))
                ) {
    ...
    render(): VNode {
        return (
            <DisplayUnit failed={this.displayFailed} bus={this.props.bus}>
                <svg
                    class="pfd-svg"
                    version="1.1"
